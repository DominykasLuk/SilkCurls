...
?>><?php
	...
	foreach ($args['icons'] as $item) {
		...
		?><div class="sc_icons_item<?php echo !empty($item['link']) ? ' sc_icons_item_linked' : ''; ?>"
			

			<?php
			if (!empty($item['image'])) { 
				$item['image'] = trx_addons_get_attachment_url($item['image']);
					echo ' style="'							
						. (!empty($item['image']) ? 'background-image:url('.esc_url($item['image']).');' : '')
						. '"'; 
			}
		?>>
				...
				if (!empty($icon)) {
					...
					?>><?php
						if (!empty($svg)) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'cleanskin'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						} else {
