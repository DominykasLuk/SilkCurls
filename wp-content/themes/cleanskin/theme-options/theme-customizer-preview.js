...
( function( $ ) {
	...
	var $style = $( '#cleanskin-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		var style_container = '<' + 'style type="text/css" id="cleanskin-customizer-inline-css" />',
			$theme_css      = $( '#cleanskin-main-inline-css' );
		if ($theme_css.length == 0) {
			$theme_css = $( '#cleanskin-main-css' );
		}
		if ($theme_css.length == 0) {
		...
		$style = $( '#cleanskin-customizer-inline-css' );
	}
			...
			api.preview.bind(
				'refresh-other-controls', function( obj ) {
					...
					if (id.indexOf( 'body_style' ) == 0) {
						...
						$( '.sc_layouts_logo .logo_text' ).html( cleanskin_prepare_macros( val ) );

					} else if (id == 'blogdescription') {
						$( '.sc_layouts_logo .logo_slogan' ).html( cleanskin_prepare_macros( val ) );

					} else if (id == 'copyright') {
						$( '.copyright_text' ).html( cleanskin_prepare_macros( val ) );
						...
						if (id == 'front_page_bg_image') {
							...
							if (color_type == 'none') {
								...
								color = typeof cleanskin_color_schemes[scheme] != 'undefined' ? cleanskin_color_schemes[scheme]['colors']['bg_color'] : '';
							}
							$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : cleanskin_hex2rgba( color, mask ) ) );
							...
							.height( 'auto' )
							.toggleClass( 'cleanskin-full-height sc_layouts_flex sc_layouts_columns_middle', val );
							$( window ).trigger( 'resize' );
						}
					}
				}
