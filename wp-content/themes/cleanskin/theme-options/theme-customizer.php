...
if ( ! function_exists( 'cleanskin_sanitize_html' ) ) {
	function cleanskin_sanitize_html( $value ) {
		return empty( $value ) ? $value : wp_kses( $value, 'cleanskin_kses_content' );
	}
}
		...
		<div class="cleanskin_customizer_message">
			...
			echo wp_kses_data(
				sprintf(
					...
					is_customize_preview()
						? esc_html__( 'ThemeREX Addons', 'cleanskin' )
						: sprintf(
	...
	function cleanskin_customizer_css_template() {
		...
		if ( count( $schemes ) > 0 ) {
			foreach ( $schemes as $scheme ) {
				cleanskin_show_layout(
					...
					'<' . esc_attr( $tmpl_holder ) . ' type="text/html" id="tmpl-cleanskin-color-scheme-' . esc_attr( $scheme ) . '">',
		...
		if ( is_array( $fonts ) && count( $fonts ) > 0 ) {
			...
			cleanskin_show_layout(
				...
				'<' . esc_attr( $tmpl_holder ) . ' type="text/html" id="tmpl-cleanskin-fonts">',
				'</' . esc_attr( $tmpl_holder ) . '>'
			);
		}
		...
		if ( is_array( $vars ) && count( $vars ) > 0 ) {
			...
			cleanskin_show_layout(
				...
				'<' . esc_attr( $tmpl_holder ) . ' type="text/html" id="tmpl-cleanskin-vars">',
				'</' . esc_attr( $tmpl_holder ) . '>'
			);
		}
