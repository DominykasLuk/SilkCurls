/* global cleanskin_color_schemes, cleanskin_dependencies, Color */
...
( function( api ) {
	...
	for (var i in cleanskin_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'cleanskin-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'cleanskin-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'cleanskin-vars' );
	...
	api.bind(
		'ready', function() {
			...
			// Add 'reset' button
			jQuery( '#customize-header-actions #save' ).before( '<input type="button" class="button customize-action-reset" value="' + cleanskin_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' ).on(
				'click', function(e) {
					if (confirm( cleanskin_customizer_vars['msg_reset_confirm'] )) {
			...
			if (cleanskin_customizer_vars && cleanskin_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( cleanskin_customizer_vars['theme_name_suffix'] );
			}

			// Blur the "load fonts" fields - regenerate options lists in the font-family controls
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' ).on( 'focusout', cleanskin_customizer_update_load_fonts );

			// Click on the actions button
			jQuery( '#customize-theme-controls .control-section .customize-control-button input[type="button"]' ).on( 'click', cleanskin_customizer_actions );

			// Check dependencies in the each section on ready
			jQuery( '#customize-theme-controls .control-section' ).each(
				function () {
					cleanskin_customizer_check_dependencies( jQuery( this ) );
				}
			);
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' ).on(
				'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							cleanskin_customizer_check_dependencies( section );
						}
					}
				}
			);
	...
	api.bind(
		'change', function(obj) {
			...
			cleanskin_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
			cleanskin_customizer_refresh_preview( obj );
		}
	);
	...
	api.bind(
		'refresh_schemes', function() {
			...
			for (var i in cssTemplate) {
				...
				i = i.replace( 'scheme_', '' );
				if (typeof cleanskin_color_schemes[i] === 'undefined') {
					delete cssTemplate['scheme_' + i];
					jQuery( '#tmpl-cleanskin-color-scheme-' + i ).remove();
				} else if (tpl_idx == '') {
					tpl_idx     = i;
					tpl_content = jQuery( '#tmpl-cleanskin-color-scheme-' + i ).html().trim();
				}
			}
			// Add new templates
			...
			for (var i in cleanskin_color_schemes) {
				if (typeof cssTemplate['scheme_' + i] === 'undefined') {
					jQuery( '#tmpl-cleanskin-color-scheme-' + tpl_idx )
					.clone()
					.attr( 'id', 'tmpl-cleanskin-color-scheme-' + i )
					.html( tpl_content.replace( regex, '.scheme_' + i ) )
					.insertAfter( jQuery( '#tmpl-cleanskin-color-scheme-' + tpl_idx ) );
					cssTemplate['scheme_' + i] = wp.template( 'cleanskin-color-scheme-' + i );
				}
			}
		}
	...
	for (var action in cleanskin_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (cleanskin_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = cleanskin_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = cleanskin_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function cleanskin_customizer_get_field_value(fld) {
	...
	function cleanskin_customizer_check_dependencies(cont) {
		cont.find( '.customize-control' ).each(
			function() {
				...
				var fld    = null, val = '', i;
				var depend = false;
				for (fld in cleanskin_dependencies) {
					if (fld == id) {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = cleanskin_customizer_get_field_value( fld );
					...
					if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
						ctrl.show().removeClass( 'cleanskin_options_no_use' );
					} else {
						ctrl.hide().addClass( 'cleanskin_options_no_use' );
					}
				}
				...
				if (false && id == 'color_scheme') {
					...
					if (fld.length > 0) {
						val     = cleanskin_customizer_get_field_value( fld );
						var num = 0;
						for (i in cleanskin_color_schemes) {
							num++;
							if (i == val) {
								break;
							}
	...
	function cleanskin_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( cleanskin_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					cleanskin_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in cleanskin_theme_fonts) {
					for (rule in cleanskin_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							cleanskin_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
			...
			if (processed || forceUpdateCSS) {
				cleanskin_customizer_update_css();
			}
		}
	...
	// Actions buttons
	function cleanskin_customizer_actions(e) {
	...
	function cleanskin_customizer_update_theme_vars(opt, value) {
		cleanskin_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function cleanskin_customizer_update_theme_fonts(opt, rule, value) {
		cleanskin_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function cleanskin_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= cleanskin_customizer_vars['max_load_fonts']; i++) {
	...
	function cleanskin_customizer_update_css() {
		...
		var vars = cleanskin_customizer_add_theme_vars( cleanskin_theme_vars );

		// Make styles and add its to the css
		css += cleanskin_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = cleanskin_customizer_add_theme_fonts( cleanskin_theme_fonts );

		// Make styles and add its to the css
		css += cleanskin_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in cleanskin_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( cleanskin_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in cleanskin_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in cleanskin_color_schemes[scheme].colors) {
				colors[i] = cleanskin_color_schemes[scheme].colors[i];
			}
			...
			var tmpl = jQuery( '#tmpl-cleanskin-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function cleanskin_customizer_add_theme_colors(colors) {
		if (cleanskin_additional_colors) {
			var clr = '', v = '';
			for (var k in cleanskin_additional_colors) {
				v   = cleanskin_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + cleanskin_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + cleanskin_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + cleanskin_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function cleanskin_customizer_add_theme_vars(vars) {
		...
		for (var i in vars) {
			if (i == 'rad') {
				...
				rez['rad']   = cleanskin_customizer_prepare_css_value( vars['rad'] );
				...
				rez['page']        = cleanskin_customizer_prepare_css_value( vars['page'] );
				rez['sidebar']     = cleanskin_customizer_prepare_css_value( vars['sidebar'] );
				rez['gap']         = cleanskin_customizer_prepare_css_value( vars['gap'] );
				rez['content']     = cleanskin_customizer_prepare_css_value( vars['page'] - vars['sidebar'] - vars['gap'] );
				rez['sidebar_gap'] = cleanskin_customizer_prepare_css_value( vars['sidebar'] + vars['gap'] );
				rez['sidebar_prc'] = vars['sidebar'] / vars['page'];
				rez['gap_prc']     = vars['gap'] / vars['page'];
				rez['page_boxed']  = cleanskin_customizer_prepare_css_value( vars['page'] + 2 * 60 );
				rez['page_1_1']    = cleanskin_customizer_prepare_css_value( vars['page'] );
				rez['page_1_2']    = cleanskin_customizer_prepare_css_value( vars['page'] / 2 );
				rez['page_1_3']    = cleanskin_customizer_prepare_css_value( vars['page'] / 3 );
				rez['page_2_3']    = cleanskin_customizer_prepare_css_value( vars['page'] / 3 * 2 );
				rez['page_1_4']    = cleanskin_customizer_prepare_css_value( vars['page'] / 4 );
				rez['page_3_4']    = cleanskin_customizer_prepare_css_value( vars['page'] / 4 * 3 );
				var margins        = [10,20,30,40,60];
				for (var m = 0; m < margins.length; m++) {
					rez['page' + margins[m]]    = cleanskin_customizer_prepare_css_value( vars['page'] + margins[m] );
					rez['content' + margins[m]] = cleanskin_customizer_prepare_css_value( vars['page'] - vars['gap'] - vars['sidebar'] + margins[m] );
				}
			}
		}
		return rez;
	...
	// Add ed to css value
	function cleanskin_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function cleanskin_customizer_prepare_html_value(val) {
